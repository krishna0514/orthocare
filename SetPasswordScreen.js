...
const SetPasswordWithImageScreen = ({ route }) => {
  ...
  const { doctorId, imageUri } = route.params; // Receive imageUri here
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Add image data to form data if an image is selected
      if (image) {
      ...
      const response = await fetch('http://172.18.30.176/demo/setpassword.php', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topContainer}>
        ...
        <Text style={styles.doctorIdText}>Doctor ID: {doctorId}</Text>
      </View>
